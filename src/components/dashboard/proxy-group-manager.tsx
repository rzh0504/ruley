...
import { GitBranchIcon, LibraryIcon, PlusIcon, RotateCcwIcon, Trash2Icon } from "lucide-react";
...
const groupModes: { value: ProxyGroupType; label: string; description: string }[] = [
...
const policyOptionDefinitions: { value: PolicyOption; label: string; description: string }[] = [
  { value: "main", label: "节点选择组", description: "加入入口策略组，通常是 🚀 节点选择" },
  ...
  { value: "matched", label: "匹配节点", description: "加入正则匹配到的真实节点" },
];

const colorVariants: Record<ProxyGroupTemplate["color"], "default" | "info" | "success" | "warning" | "error"> = {
  blue: "info",
  green: "success",
  purple: "default",
  red: "error",
...
const createTemplateGroup = (template: ProxyGroupTemplate): ProxyGroupTemplate => ({
  ...template,
  ruleLinks: getDefaultRuleLinks(template),
});

const isCustomGroup = (group: ProxyGroupTemplate) => group.id.startsWith("custom-");

const getPreset = (group: ProxyGroupTemplate) => PROXY_GROUP_TEMPLATES.find((template) => template.id === group.id);

const normalizeIdPart = (value: string) => value.trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9\-_\u4e00-\u9fa5]/g, "");
...
export function ProxyGroupManager({
  ...
  const [customDraft, setCustomDraft] = useState({
    ...
    policyOptions: getDefaultPolicyOptions({ id: "custom-draft", type: "select" }),
    filter: "^(.*)$",
    ruleLinks: "",
    dialerProxy: "",
  });
  ...
  const activeIds = useMemo(() => new Set(groups.map((group) => group.id)), [groups]);

  const updateGroup = (groupId: string, updates: Partial<ProxyGroupTemplate>) => {
    onGroupsChange(groups.map((group) => group.id === groupId ? { ...group, ...updates } : group));
  };
  ...
  const togglePreset = (template: ProxyGroupTemplate) => {
    ...
    const nextGroups = [...groups, createTemplateGroup(template)].sort((a, b) => {
  ...
  const createCustomGroup = () => {
    ...
    if (activeIds.has(id)) id = `custom-${idBase}-${crypto.randomUUID().slice(0, 8)}`;
    ...
    <Card className="h-[44rem] min-h-0">
      <CardHeader className="gap-3 md:grid-cols-[1fr_auto]">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant={mode === "custom" ? "secondary" : "outline"} onClick={() => setMode(mode === "custom" ? "edit" : "custom")}>
        ...
        <aside className="min-h-0 h-full overflow-auto flex flex-col gap-2 rounded-xl border bg-muted/40 p-2">
          {groups.map((group) => (
            <button
              key={group.id}
              type="button"
              onClick={() => { setSelectedId(group.id); setMode("edit"); }}
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-muted-foreground text-xs">{group.filter || "无节点筛选"}</div>
                  </div>
                </div>
                <Badge variant={colorVariants[group.color]}>{isCustomGroup(group) ? "自定义" : group.type}</Badge>
              </div>
            </button>
          ))}
        </aside>
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <span className="font-medium"><span aria-hidden="true">{template.icon}</span> {template.name}</span>
...
function CustomGroupForm({
  ...
  draft: { icon: string; name: string; type: ProxyGroupType; policyOptions: PolicyOption[]; filter: string; ruleLinks: string; dialerProxy: string };
  groups: ProxyGroupTemplate[];
  onChange: (draft: { icon: string; name: string; type: ProxyGroupType; policyOptions: PolicyOption[]; filter: string; ruleLinks: string; dialerProxy: string }) => void;
  onCancel: () => void;
  onCreate: () => void;
}) {
  return (
    <section className="h-full min-h-0 overflow-auto flex flex-col gap-4 rounded-xl border p-4">
      <div className="grid gap-3 md:grid-cols-[5rem_1fr]">
        <label className="flex flex-col gap-2 text-sm font-medium">
          图标
          <Input nativeInput value={draft.icon} onChange={(event) => onChange({ ...draft, icon: event.target.value })} />
        </label>
        <label className="flex flex-col gap-2 text-sm font-medium">
          组名称
          <Input nativeInput value={draft.name} onChange={(event) => onChange({ ...draft, name: event.target.value })} placeholder="例如 Netflix" />
        </label>
      </div>
      <GroupModePicker value={draft.type} onChange={(type) => onChange({
        ...draft,
        type,
        policyOptions: getDefaultPolicyOptions({ id: "custom-draft", type }),
      })} />
      <PolicyOptionsPicker
      ...
      />
      <FilterPicker value={draft.filter} onChange={(filter) => onChange({ ...draft, filter })} />
      <label className="flex flex-col gap-2 text-sm font-medium">
        链式代理
        <StringSelect
          items={[
            { label: "不使用链式代理", value: "" },
            ...groups.map((group) => ({ label: `${group.icon} ${group.name}`, value: group.id })),
          ]}
      ...
      <label className="flex flex-col gap-2 text-sm font-medium">
        外部规则链接
        <Textarea value={draft.ruleLinks} onChange={(event) => onChange({ ...draft, ruleLinks: event.target.value })} placeholder="每行一个 .mrs/.yaml 规则链接，可追加 ,no-resolve" className="min-h-28" />
      </label>
      <div className="flex justify-end gap-2">
        <Button variant="ghost" onClick={onCancel}>取消</Button>
        <Button onClick={onCreate} disabled={!draft.name.trim()}>创建策略组</Button>
      </div>
    </section>
  );
}
  ...
  return (
    <section className="h-full min-h-0 overflow-auto flex flex-col gap-4 rounded-xl border p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <Badge variant={preset ? "info" : "default"}>{preset ? "预设" : "自定义"}</Badge>
            {group.dialerProxy && <Badge variant="warning"><GitBranchIcon aria-hidden="true" />链式代理</Badge>}
          </div>
          <p className="text-muted-foreground text-sm">{group.desc}</p>
        </div>
        <div className="flex gap-2">
          {preset && <Button variant="outline" size="sm" onClick={onReset}><RotateCcwIcon aria-hidden="true" />重置</Button>}
          <Button variant="destructive-outline" size="sm" onClick={onRemove} disabled={groups.length <= 1}><Trash2Icon aria-hidden="true" />移除</Button>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-[5rem_1fr]">
        <label className="flex flex-col gap-2 text-sm font-medium">
          图标
          <Input nativeInput value={group.icon} onChange={(event) => onUpdate({ icon: event.target.value })} />
        </label>
        <label className="flex flex-col gap-2 text-sm font-medium">
          组名称
          <Input nativeInput value={group.name} onChange={(event) => onUpdate({ name: event.target.value })} />
        </label>
      </div>

      {lockedType ? (
        <div className="rounded-xl border bg-muted/50 p-3 text-sm text-muted-foreground">该基础策略组的代理组模式已锁定为 <strong>{group.type}</strong>，避免生成无效配置。</div>
      ) : (
        <GroupModePicker value={group.type} onChange={(type) => onUpdate({
          type,
        ...
        <StringSelect
          items={[
            ...
            ...groups
              ...
              .map((item) => ({ label: `${item.icon} ${item.name}`, value: item.id })),
          ]}
      ...
      <FilterPicker value={group.filter} onChange={(filter) => onUpdate({ filter })} />

      <label className="flex flex-col gap-2 text-sm font-medium">
        外部规则链接
        <Textarea value={ruleLinks} onChange={(event) => onUpdate({ ruleLinks: event.target.value })} placeholder="每行一个 .mrs/.yaml 规则链接，可追加 ,no-resolve" className="min-h-32 font-mono text-xs" />
      </label>
    </section>
  );
}

function GroupModePicker({ value, onChange }: { value: ProxyGroupType; onChange: (value: ProxyGroupType) => void }) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex flex-col gap-1">
        <span className="text-sm font-medium">代理组模式</span>
        <span className="text-muted-foreground text-xs">对应 Mihomo <code>proxy-groups[].type</code>，每个代理组只能有一种模式。</span>
      </div>
      <div className="grid gap-2 md:grid-cols-3">
        {groupModes.map((strategy) => (
          ...
          >
            ...
            <div className="text-muted-foreground text-xs">{strategy.description}</div>
          </button>
        ))}
      </div>
    </div>
...
function PolicyOptionsPicker({
  ...
  const selected = new Set(value.length > 0 ? value : getDefaultPolicyOptions({ id: groupId, type: groupType }));
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl border bg-muted/30 p-3">
      <div className="flex flex-col gap-1">
        ...
        <span className="text-muted-foreground text-xs">对应 Mihomo <code>proxy-groups[].proxies</code>，可多选。匹配节点会根据下方正则动态加入真实节点。</span>
      </div>
...
function FilterPicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  const toggleFilter = (filter: string) => {
    ...
    const active = new Set(commonFilters.filter((item) => item.value !== "^(.*)$" && value.includes(item.value)).map((item) => item.value));
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border bg-muted/30 p-3">
      ...
      <label className="flex flex-col gap-2 text-sm font-medium">
        进阶正则匹配
        <Input nativeInput value={value} onChange={(event) => onChange(event.target.value)} className="font-mono" />
      </label>
    </div>
  );
}
...
export function StringSelect({
  ...
  return (
    <Select
      ...
      items={items}
      onValueChange={(item: StringSelectItem | null) => onChange(item?.value || "")}
      value={selected}
    >
      <SelectTrigger>
